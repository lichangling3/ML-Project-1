...
The aim of this project is to learn to use the concepts presented in the lectures and practiced in the labs on a
real-world dataset, start to finish. In the following sections, we will present every step taken such as exploratory data analysis to understand the dataset and the features, before applying feature processing and engineering to clean the dataset and extract more meaningful information. We implemented the machine learning methods from the set of regression analysis such as linear regression, ridge regression and logistic regression using gradient descent, stochastic gradient descent or normal equations. We analyzed the results produced by these different methods and continuously improved the models, by finally choosing the best one. This project was done in the context of a recent popular machine learning challenge - finding the Higgs boson -, using original data from CERN~\cite{higgs}.
...
\end{itemize}
\ \\
Then, after loading the data from the file \texttt{train.csv} using the function \texttt{load\_csv\_data} from the file \texttt{ proj1\_helpers.py}, we had to make sure that the labels correspond either to 1 (which corresponds to s) and to -1 (for b). Then, we inspected the size of the dataset which contains 250'000 data samples with 30 feature columns. It reassures us on the fact that we will not face the challenges of a high-dimensional data. Then, we analyzed our data as explained in the following subsections.
\ \\
\begin{description}
\item[Inspecting the features] \ \\
